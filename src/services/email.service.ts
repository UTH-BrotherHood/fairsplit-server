  ...
  async sendVerificationCode(to: string, username: string, code: string): Promise<boolean> {
    ...
    const fallbackTemplate = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="text-align: center; margin: 20px 0;">
          <div style="font-size: 32px; font-weight: bold; letter-spacing: 8px; padding: 20px; background-color: #f5f5f5; border-radius: 4px;">{{code}}</div>
        </div>
    ...
    try {
      // Try to get the template, with fallback
      const template = await this.getEmailTemplate('verification-code', fallbackTemplate)
      ...
      if (!success) {
        logger.error(`Failed to send verification code email to ${to} after all retries`, '', '', {
          error: lastError instanceof Error ? lastError.message : 'Unknown error',
          recipient: to
        })
        throw lastError
      ...
      logger.error(`Failed to send verification code email to ${to}`, '', '', {
        error: error instanceof Error ? error.message : 'Unknown error',
        recipient: to
      })
      throw error
