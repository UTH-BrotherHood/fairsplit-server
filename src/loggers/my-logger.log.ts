...
class MyLogger {
  ...
  constructor() {
    ...
    const formatPrint = winston.format.printf((info: winston.Logform.TransformableInfo) => {
    ...
    const commonRotateConfig = {
      ...
      maxSize: '1m',
      maxFiles: '14d',
      format: winston.format.combine(winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), formatPrint)
    }
   ...
   */
  public info(message: string, context: string = '', requestId: string = '', metadata: Record<string, any> = {}): void {
   ...
   * @param metadata - Thông tin bổ sung
   */
  public warn(message: string, context: string = '', requestId: string = '', metadata: Record<string, any> = {}): void {
